...
public class ConsumerService {
    ...
    @Autowired
    private ContaServiceRedisSaque contaServiceRedisSaque;
...
//
//        contaServiceRedisSaque.criarSaquesGratuitosNoRedis(contaEntidade);
//    }

    @KafkaListener(topics = "novoSaque",groupId = GRUPO_ID, containerFactory = "consumerFactoryOperacao")//Operacao - Saque
    public void listen(OperacaoEntidade operacaoEntidade) {
        logger.info("Messagem recebida" + operacaoEntidade.toString());

        contaServiceRedisSaque.atualizarSaquesGratuitosNoRedis(operacaoEntidade);
    }
}
